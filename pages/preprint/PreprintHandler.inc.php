    ...
    public function view($args, $request)
    {
        ...
        if (empty($this->galley)) {
            ...
            if (!HookRegistry::call('PreprintHandler::view', [&$request, &$preprint, $publication])) {
                ...
                if ($preprint->getStatus() == Submission::STATUS_PUBLISHED && !$request->isDNTSet()) {
                    $file = 'debug.txt';
                    $current = file_get_contents($file);
                    $current .= print_r("++++ view event ++++\n", true);
    ...
    public function download($args, $request)
    {
        ...
        if ($this->galley->getRemoteURL()) {
            ...
            if (!HookRegistry::call('PreprintHandler::download', [$this->preprint, &$this->galley, &$this->fileId])) {
                ...
                if ($this->preprint->getStatus() == Submission::STATUS_PUBLISHED && $this->galley->getData('submissionFileId') == $this->fileId && !$request->isDNTSet()) {
                    ...
                    event(new UsageEvent($assocType, $this->fileId, $this->preprint->getContextId(), $this->preprint->getId(), $this->galley->getId(), $mimetype));
                }
                $returner = true;
