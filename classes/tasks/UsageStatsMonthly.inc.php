...
class UsageStatsMonthly extends ScheduledTask
{
    ...
    public function executeActions(): bool
    {
        ...
        DB::statement(
			...
			SELECT sd.context_id, sd.submission_id, COALESCE(sd.country, ''), COALESCE(sd.region, ''), COALESCE(sd.city, ''), {$dateFormatting} as month, SUM(sd.metric_investigations), SUM(sd.metric_investigations_unique), SUM(sd.metric_requests), SUM(sd.metric_requests_unique) FROM metrics_counter_submission_geo_daily sd WHERE {$dateFormatting} <> ? GROUP BY sd.context_id, sd.submission_id, sd.country, sd.region, sd.city, {$dateFormatting}
			",
            [$currentMonth]
        );
        if ($site->getData('geoUsageStatsKeepDaily') == 0) {
            DB::statement("DELETE FROM metrics_counter_submission_geo_daily sd WHERE {$dateFormatting} <> ?", [$currentMonth]);
        }
...
if (!PKP_STRICT_MODE) {
    class_alias('\APP\task\UsageStatsMonthly', '\UsageStatsMonthly');
}
