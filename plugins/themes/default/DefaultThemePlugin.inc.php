...
class DefaultThemePlugin extends ThemePlugin
{
    ...
    public function register($category, $path, $mainContextId = null)
    {
        ...
        if ($success && $this->isActive() && ($this->getOption('usageStatsDisplay') !== 'none')) {
            HookRegistry::register('Templates::Preprint::Main', [$this, 'displayUsageStatsGraph']);
        }
        return $success;
    }
    ...
    /**
     * Add usage statistics graph to preprint view page
     *
     ...
     * ]
     */
    public function displayUsageStatsGraph(string $hookName, array $params)
    {
        $smarty = & $params[1];
        ...
        $this->_addJavascriptData($this->_getAllDownloadsStats($submissionId), $submissionId);
     ...
     */
    private function _addJavascriptData(array $statsByMonth, int $submissionId)
    {
        ...
        $templateMgr->addJavaScript(
            ...
            [
                'inline' => true,
                'contexts' => 'frontend-preprint-view',
            ]
        );
    }
    ...
    private function _loadJavascript()
    {
        ...
        // Register Chart.js on the frontend preprint view
        $templateMgr->addJavaScript(
            'chartJS',
     ...
     */
    private function _getAllDownloadsStats(int $submissionId): array
    {
        $cache = CacheManager::getManager()->getCache('downloadStats', $submissionId, [$this, '_downloadStatsCacheMiss']);
     ...
     */
    public function _downloadStatsCacheMiss(FileCache $cache, int $submissionId): array
    {
        $request = Application::get()->getRequest();
     ...
     */
    private function _getColor(int $num): string
    {
